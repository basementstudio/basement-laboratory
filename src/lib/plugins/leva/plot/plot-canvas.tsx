...
import { Canvas, Dot, ToolTip, Wrapper } from './styled-plot'

type PlotCanvasProps = { value: InternalPlot; settings: InternalPlotSettings }

export const PlotCanvas = React.memo(
  ({ value: expr, settings }: PlotCanvasProps) => {
    const { boundsX, boundsY } = settings

    console.log('RENDER PLOT CANVAS')
    ...
    const drawPlot = useCallback(
      (_canvas: HTMLCanvasElement, _ctx: CanvasRenderingContext2D) => {
        ...
        for (let i = 0; i < width; i++) {
          ...
          const v = expr(x)
          if (v < canvasBoundsY.current.minY && v !== -Infinity) {
            canvasBoundsY.current.minY = v
          }
          if (v > canvasBoundsY.current.maxY && v !== Infinity) {
            canvasBoundsY.current.maxY = v
          }
        ...
        for (let i = 0; i < width; i++) {
          const v = invertedRange(
            range(
              points[i],
              canvasBoundsY.current.minY,
              canvasBoundsY.current.maxY
            ),
            height - 5,
            5
          )
    ...
    const bind = useMove(({ xy: [x], first }) => {
      ...
      const relY = clamp(
        yPositions.current[i * window.devicePixelRatio] /
