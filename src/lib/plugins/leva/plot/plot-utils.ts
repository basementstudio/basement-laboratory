...
import { ForeignExpression, PlotInput } from './plot-types'
...
export function isForeignExpression(expr: PlotInput['expression']) {
  return typeof expr === 'function' || Array.isArray(expr)
}

export function parseForeignExpression(expression: ForeignExpression) {
  if (Array.isArray(expression)) {
    const func = expression[1]
    const symbols = expression[0].split(',').map((s) => s.trim())
  ...
  for (const key in scope) {
    ...
    const s =
      typeof scope[key] === 'function'
        ? scope[key].__parsedScoped.toString()
        : scope[key]
